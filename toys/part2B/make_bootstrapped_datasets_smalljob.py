...
smc_rootfile = '/gpfs/home/belle/varghese/IPHC/B_Kspipigamma_belle/toys/parent_MCsamples/%s_1M_correct_wRMVA_wB2MVA_sel_BCS_branches.root'%SMC_sample
...
df_rare_sigscf = df_rare.query('issgnevt == 1')#only to calculate true yield
...
true_yields = {'gmc_yield' : len(df_gmc)/6.0,
               ...
               'sigscf_yield' : len(df_rare_sigscf) / 50.0}
...
dataset_sigscf_yields = np.random.poisson(true_yields['sigscf_yield'], number_toys)

#generate each toy-dataset
for i, (gmc_yield, rarebkg_yield, sigscf_yield) in enumerate(zip(dataset_gmc_yields, dataset_rarebkg_yields, dataset_sigscf_yields)):

	df_toy_gmc = df_gmc.sample(gmc_yield, replace=True)
	df_toy_rarebkg = df_rare_bkg.sample(rarebkg_yield, replace=True) 
	df_toy_sigscf = df_smc.sample(sigscf_yield, replace=True)	

	df_toy = pd.concat([df_toy_gmc, df_toy_rarebkg, df_toy_sigscf], ignore_index=True)
