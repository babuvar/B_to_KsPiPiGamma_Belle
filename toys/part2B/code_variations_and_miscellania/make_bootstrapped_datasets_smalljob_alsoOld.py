...
This script creates toys datasets with SCF from the MC-rare.
...
df_rare_scf = df_rare.query('issignal != 1 & issgnevt == 1')
df_rare_sig = df_rare.query('issignal == 1')#only to calculate true yield
df_smc = uproot.open(smc_rootfile)['h1'].arrays([split_prop(i)[0] for i in variables], library='pd')
df_smc_sig = df_smc.query('issignal == 1')

# estimate true - yields
true_yields = {'gmc_yield' : len(df_gmc)/6.0,
               'rare_bkg_yield' : len(df_rare_bkg)/50.0,
               'scf_yield' : len(df_rare_scf) / 50.0,
               'sig_yield' : len(df_rare_sig) / 50.0}
...
dataset_scf_yields = np.random.poisson(true_yields['scf_yield'], number_toys)
dataset_sig_yields = np.random.poisson(true_yields['sig_yield'], number_toys)

#generate each toy-dataset
for i, (gmc_yield, rarebkg_yield, scf_yield, sig_yield) in enumerate(zip(dataset_gmc_yields, dataset_rarebkg_yields, dataset_scf_yields, dataset_sig_yields)):

	df_toy_gmc = df_gmc.sample(gmc_yield, replace=True)
	df_toy_rarebkg = df_rare_bkg.sample(rarebkg_yield, replace=True) 
	df_toy_scf = df_rare_scf.sample(scf_yield, replace=True)
	df_toy_sig = df_smc_sig.sample(sig_yield, replace=True)	

	df_toy = pd.concat([df_toy_gmc, df_toy_rarebkg, df_toy_scf, df_toy_sig], ignore_index=True)
