...
def plot_linearity(n, metric, e_metric, inputS, parname, metricname, out_dir):
	...
	gr.SetTitle('%s-pull %s vs input-S; Input S; Pull'%(parname, metricname)); gr.SetMarkerColor(4); gr.SetMarkerStyle(21)
...
if __name__ == '__main__':
	...
	pullmean_Sp = []; e_pullmean_Sp = []; pullsig_Sp = []; e_pullsig_Sp = []
	pullmean_Sm = []; e_pullmean_Sm = []; pullsig_Sm = []; e_pullsig_Sm = []
	pullmean_A = []; e_pullmean_A = []; pullsig_A = []; e_pullsig_A = []
	
	
	#Pull distributions for S-plus
	for i, (rfname, Trueval, S_val)  in enumerate(zip(rfnames_Sp, trueval_Sp, trueval_S)):
		pullmean, e_pullmean, pullsig, e_pullsig = plot_pulls(rfname, results_dir, Trueval, True, 'S_plus', S_val, out_dir)
		pullmean_Sp.append(pullmean); e_pullmean_Sp.append(e_pullmean)
		pullsig_Sp.append(pullsig); e_pullsig_Sp.append(e_pullsig)

	#Pull distributions for S-minus
	for i, (rfname, Trueval, S_val)  in enumerate(zip(rfnames_Sm, trueval_Sm, trueval_S)):
		pullmean, e_pullmean, pullsig, e_pullsig = plot_pulls(rfname, results_dir, Trueval, True, 'S_minus', S_val, out_dir)
		pullmean_Sm.append(pullmean); e_pullmean_Sm.append(e_pullmean)
		pullsig_Sm.append(pullsig); e_pullsig_Sm.append(e_pullsig)	
	...
	plot_linearity(7, pullmean_Sp, e_pullmean_Sp, trueval_S, 'S_plus', 'mean', out_dir)
	plot_linearity(7, pullsig_Sp, e_pullsig_Sp, trueval_S, 'S_plus', 'sigma', out_dir)
	plot_linearity(7, pullmean_Sm, e_pullmean_Sm, trueval_S, 'S_minus', 'mean', out_dir)
	plot_linearity(7, pullsig_Sm, e_pullsig_Sm, trueval_S, 'S_minus', 'sigma', out_dir)
