...
/*------------------------------------------------------------------------------*/
static const double
do_mnfit(const char *initfile, double Ain, double Sin)
{
	double fcn;
	...
	read_init(initfile);
	//Override input S and A in initfile
	observedScp = Sin; observedAcp = Ain;
	...
	double deltaS = observedScp - trueScp, deltaA = observedAcp - trueAcp;
	//cout<<"S-input = "<<observedScp<<"\t deltaS = "<<deltaS<<endl;
	//cout<<"A-input = "<<observedAcp<<"\t deltaA = "<<deltaA<<endl;
	ofstream fout;
	fout.open("output.txt", std::ios_base::app);
	fout<<observedScp<<"\t"<<deltaS<<"\t"<<observedAcp<<"\t"<<deltaA<<endl;
	fout.close();
...
main(int argc, char *argv[])
{
	...
	double Sin = atof(argv[2]);
        double Ain = atof(argv[3]);

	do_mnfit(argv[1], Ain, Sin);
