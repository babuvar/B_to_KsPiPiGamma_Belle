...
float randParVal(RooWorkspace * w, char* dimension, int toyIndex,string parameter )
{
RooAbsData *randpars = w->data(dimension);
RooRealVar * par = (RooRealVar*) (randpars->get(toyIndex)->find(parameter.c_str()) );
...
int main(int argc, char *argv[]) {
	...
	//Randomized dtres parameters
	TFile *F_dtres = new TFile("/home/belle/varghese/IPHC/B_Kspipigamma_belle/dtres_systematics/RandomizedParameters/dtresPars/rand_DTresPars.root");
        RooWorkspace *w_dtres =(RooWorkspace *)F_dtres->Get("rws_rand_dtresPars");
        float dtres_randparsigma; int dtres_item = 0, dtres_subitem = 0, expno = 31;

        for(dtres_item = 0; dtres_item <= 37; dtres_item++){
        expno = 29;
        string parname = "dtres_item" + to_string(dtres_item)+ "_" + to_string(dtres_subitem) + "_exp" + to_string(expno);
        dtres_randparsigma = randParVal(w_dtres, "data", toyindex, parname); //cout<<parname<<" = "<<dtres_randparsigma<<endl;
        //dtres_randparsigma = 0.0;//for testing
        dtres_systematics_b0(dtres_item, dtres_randparsigma, expno, 1);

        expno = 31;
        parname = "dtres_item" + to_string(dtres_item)+ "_" + to_string(dtres_subitem) + "_exp" + to_string(expno);
        dtres_randparsigma = randParVal(w_dtres, "data", toyindex, parname); //cout<<parname<<" = "<<dtres_randparsigma<<endl;
        //dtres_randparsigma = 0.0;//for testing
        dtres_systematics_b0(dtres_item, dtres_randparsigma, expno, 1);
        }
