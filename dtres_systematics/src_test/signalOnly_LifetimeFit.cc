...
int main(int argc, char *argv[]) {
	...
	*/
	...
	RooRealVar inexp("inexp", "inexp", 1, 69);
...
//----------------------------------------------------------------------------------------------//
	...
	//dt
	RooDtLifetime* sigLifeDT = new RooDtLifetime("sigLifeDT", "sigLifeDT", dt, tauB, inexp, CosThetaB, ecms, ntrk_sig, z_err_sig, chisq_tracks_sig, dgf_tracks_sig, ntrk_tag, z_err_tag, chisq_tracks_tag, dgf_tracks_tag, keeptagl_tag, mc, addoutlier, 1.0);
...
//----------------------------------------------------------------------------------------------//
	...
	can->SaveAs("sigOnly_lifetime.png");

	//Save fit results
	
	//To individual fit file
	//RooArgSet(fullFit->floatParsFinal()).writeToFile("lifetime_result.txt");
