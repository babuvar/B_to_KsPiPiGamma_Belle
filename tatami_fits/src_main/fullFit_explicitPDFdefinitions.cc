...
//------------------------------------------------------------------------------------------------------
	...
	*/
	const double tau_bb_missFSP = 1.4896;
        RooDtCPSignal bb_missFSP_DT("bb_missFSP_DT", "bb_missFSP_DT", dt, RooConst(0.0), RooConst(0.0),
                        ...
                        keeptagl_tag, iflavor, w, dw, delta_m, tau_bb_missFSP, true, true, 1.0);
...
//------------------------------------------------------------------------------------------------------
	...
	*/
	const double tau_scf = 1.0906;
        ...
        RooDtCPSignal scfDT("scfDT", "scfDT", dt, S_scf, A_scf,
                        inexp, CosThetaB, ecms, ntrk_sig, z_err_sig, chisq_tracks_sig,
                        dgf_tracks_sig, ntrk_tag, z_err_tag, chisq_tracks_tag, dgf_tracks_tag,
                        keeptagl_tag, iflavor, w, dw, delta_m, tau_scf, true, true, 1.0);
//------------------------------------------------------------------------------------------------------
//
...
//----------------------------------------------------------------------------------------------//
	...
	//dt
	RooDataSet* signal_set_p = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::positive && (irbin == irbin::5 || irbin == irbin::6)"));
	RooDataSet* signal_set_n = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::negative && (irbin == irbin::5 || irbin == irbin::6)"));
	
	RooPlot *xframe_dt = dt.frame(Bins(10), Title(""));
	signal_set_p->plotOn(xframe_dt, DataError(RooAbsData::SumW2), MarkerColor(kBlue), LineColor(kBlue));
	combined3D->plotOn(xframe_dt, ProjWData(*signal_set_p), LineColor(kBlue), NumCPU(10, 0));
        signal_set_n->plotOn(xframe_dt, DataError(RooAbsData::SumW2), MarkerColor(kRed), LineColor(kRed));
        combined3D->plotOn(xframe_dt, ProjWData(*signal_set_n), LineColor(kRed), NumCPU(10, 0));
