...
int main(int argc, char *argv[]) {
	...
	string canvasName2 = canvasName.erase(canvasName.length() - 5);
	canvasName2 = canvasName2+"_2.root";
...
//------------------------------------------------------------------------------------------------------
	...
	//Signal-pars
        ...
        RooRealVar S_plus("S_plus", "S_plus",  -4., 4.);
...
//----------------------------------------------------------------------------------------------//
	...
	RooDataSet* dalitz_p = dynamic_cast<RooDataSet*> (data->reduce("dalitzCategory==dalitzCategory::Plus"));
	RooDataSet* dalitz_m = dynamic_cast<RooDataSet*> (data->reduce("dalitzCategory==dalitzCategory::Minus"));

	
	//Plotting
	//Mbc
	//Dalitz-plus
	RooPlot *xframe_mbc_p = mbc.frame(Bins(40), Title(""));
	dalitz_p->plotOn(xframe_mbc_p, DataError(RooAbsData::SumW2));
	combined3D->plotOn(xframe_mbc_p, ProjWData(*dalitz_p), Components("signalMBC_Dp"), LineColor(kRed), LineStyle(kDashed));
	combined3D->plotOn(xframe_mbc_p, ProjWData(*dalitz_p), LineColor(kBlue));
	RooHist* pull_mbc_p = xframe_mbc_p->pullHist() ;
	RooPlot* f_pull_mbc_p = mbc.frame(Title("")) ;
	f_pull_mbc_p->addPlotable(pull_mbc_p,"BX0") ;
	f_pull_mbc_p->SetMaximum(4);
	f_pull_mbc_p->SetMinimum(-4);
	f_pull_mbc_p->GetYaxis()->SetLabelSize(0.11);
        f_pull_mbc_p->GetXaxis()->SetLabelSize(0.11);
	//Dalitz minus
        RooPlot *xframe_mbc_m = mbc.frame(Bins(40), Title(""));
        dalitz_m->plotOn(xframe_mbc_m, DataError(RooAbsData::SumW2));
        combined3D->plotOn(xframe_mbc_m, ProjWData(*dalitz_m), Components("signalMBC_Dm"), LineColor(kRed), LineStyle(kDashed));
        combined3D->plotOn(xframe_mbc_m, ProjWData(*dalitz_m), LineColor(kBlue));
        RooHist* pull_mbc_m = xframe_mbc_m->pullHist() ;
        RooPlot* f_pull_mbc_m = mbc.frame(Title("")) ;
        f_pull_mbc_m->addPlotable(pull_mbc_m,"BX0") ;
        f_pull_mbc_m->SetMaximum(4);
        f_pull_mbc_m->SetMinimum(-4);
        f_pull_mbc_m->GetYaxis()->SetLabelSize(0.11);
        f_pull_mbc_m->GetXaxis()->SetLabelSize(0.11);



	//dE
	//Dalitz-plus
	RooPlot *xframe_de_p = de.frame(Bins(40), Title(""));
	dalitz_p->plotOn(xframe_de_p, DataError(RooAbsData::SumW2));
	combined3D->plotOn(xframe_de_p, ProjWData(*dalitz_p), Components("signalDE_Dp"), LineColor(kRed), LineStyle(kDashed));
	combined3D->plotOn(xframe_de_p, ProjWData(*dalitz_p), LineColor(kBlue));
	RooHist* pull_de_p = xframe_de_p->pullHist() ;
        RooPlot* f_pull_de_p = de.frame(Title("")) ;
        f_pull_de_p->addPlotable(pull_de_p,"BX0") ;
        f_pull_de_p->SetMaximum(4);
        f_pull_de_p->SetMinimum(-4);
	f_pull_de_p->GetYaxis()->SetLabelSize(0.11);
        f_pull_de_p->GetXaxis()->SetLabelSize(0.11);	
	//Dalitz minus
	RooPlot *xframe_de_m = de.frame(Bins(40), Title(""));
        dalitz_m->plotOn(xframe_de_m, DataError(RooAbsData::SumW2));
        combined3D->plotOn(xframe_de_m, ProjWData(*dalitz_m), Components("signalDE_Dm"), LineColor(kRed), LineStyle(kDashed));
        combined3D->plotOn(xframe_de_m, ProjWData(*dalitz_m), LineColor(kBlue));
        RooHist* pull_de_m = xframe_de_m->pullHist() ;
        RooPlot* f_pull_de_m = de.frame(Title("")) ;
        f_pull_de_m->addPlotable(pull_de_m,"BX0") ;
        f_pull_de_m->SetMaximum(4);
        f_pull_de_m->SetMinimum(-4);
        f_pull_de_m->GetYaxis()->SetLabelSize(0.11);
        f_pull_de_m->GetXaxis()->SetLabelSize(0.11);



        RooDataSet* sigset_p_dalitzP = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::positive && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Plus"));
        RooDataSet* sigset_n_dalitzP = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::negative && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Plus"));
        RooDataSet* sigset_p_dalitzM = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::positive && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Minus"));
        RooDataSet* sigset_n_dalitzM = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && iflavor == iflavor::negative && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Minus"));
	RooDataSet* sigset_dalitzP = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Plus"));
	RooDataSet* sigset_dalitzM = dynamic_cast<RooDataSet*> (data->reduce("mbc > 5.27 && mbc < 5.29 && de > -0.05 && de < 0.05 && (irbin == irbin::5 || irbin == irbin::6) && dalitzCategory==dalitzCategory::Minus"));

	//dt
	//Dalitz-plus
	RooPlot *xframe_dt_p = dt.frame(Bins(10), Title(""));
        sigset_p_dalitzP->plotOn(xframe_dt_p, DataError(RooAbsData::SumW2), MarkerColor(kBlue), LineColor(kBlue));
        combined3D->plotOn(xframe_dt_p, ProjWData(*sigset_p_dalitzP), LineColor(kBlue), NumCPU(10, 0));
        sigset_n_dalitzP->plotOn(xframe_dt_p, DataError(RooAbsData::SumW2), MarkerColor(kRed), LineColor(kRed));
        combined3D->plotOn(xframe_dt_p, ProjWData(*sigset_n_dalitzP), LineColor(kRed), NumCPU(10, 0));
	RooPlot *asframe_p = dt.frame(Bins(10), Title(""));
	sigset_dalitzP->plotOn(asframe_p, Asymmetry(iflavor));
	combined3D->plotOn(asframe_p, Asymmetry(iflavor), ProjWData(*sigset_dalitzP), LineColor(kOrange));
	//Dalitz-minus
	RooPlot *xframe_dt_m = dt.frame(Bins(10), Title(""));
        sigset_p_dalitzM->plotOn(xframe_dt_m, DataError(RooAbsData::SumW2), MarkerColor(kBlue), LineColor(kBlue));
        combined3D->plotOn(xframe_dt_m, ProjWData(*sigset_p_dalitzM), LineColor(kBlue), NumCPU(10, 0));
        sigset_n_dalitzM->plotOn(xframe_dt_m, DataError(RooAbsData::SumW2), MarkerColor(kRed), LineColor(kRed));
        combined3D->plotOn(xframe_dt_m, ProjWData(*sigset_n_dalitzM), LineColor(kRed), NumCPU(10, 0));
	RooPlot *asframe_m = dt.frame(Bins(10), Title(""));
        sigset_dalitzM->plotOn(asframe_m, Asymmetry(iflavor));
        combined3D->plotOn(asframe_m, Asymmetry(iflavor), ProjWData(*sigset_dalitzM), LineColor(kOrange));

	//Canvases and Pads
	TCanvas* can = new TCanvas("can","can",1800,800) ; //can->Divide(2,1);
	TPad *pad1 = new TPad("pad1", "pad 1",0.0,0.0,0.25,1.0,0);
	TPad *pad2 = new TPad("pad2", "pad 2",0.25,0.0,0.50,1.0,0);
	TPad *pad3 = new TPad("pad3", "pad 3",0.50,0.0,0.75,1.0,0);
	TPad *pad4 = new TPad("pad4", "pad 4",0.75,0.0,1.0,1.0,0);
	...
	pad4->cd();
	TPad *pad41 = new TPad("pad41", "pad 41",0.0,0.2,1.0,1.0,0);
	TPad *pad42 = new TPad("pad42", "pad 42",0.0,0.0,1.0,0.2,0);
	pad41->Draw(); pad42->Draw();

	
	TCanvas* can2 = new TCanvas("can2","can2",1800,800) ; 
	TPad *tad1 = new TPad("tad1", "pad 1",0.0,0.0,0.5,0.5,0);
        TPad *tad2 = new TPad("tad2", "pad 2",0.5,0.0,1.0,0.5,0);
        TPad *tad3 = new TPad("tad3", "pad 1",0.0,0.5,0.5,1.0,0);
        TPad *tad4 = new TPad("tad4", "pad 2",0.5,0.5,1.0,1.0,0);
	tad1->Draw();   tad2->Draw();   tad3->Draw(); tad4->Draw();

	//Draw
	pad11->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_mbc_p->Draw() ;
	pad12->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_mbc_p->Draw() ;
        pad21->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_mbc_m->Draw() ;
        pad22->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_mbc_m->Draw() ;
	pad31->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_de_p->Draw() ;
	pad32->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_de_p->Draw() ;
        pad41->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_de_m->Draw() ;
        pad42->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_de_m->Draw() ;

	tad1->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_dt_p->Draw() ;
        tad2->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_dt_m->Draw() ;
        tad3->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); asframe_p->Draw() ;
        tad4->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); asframe_m->Draw() ;
