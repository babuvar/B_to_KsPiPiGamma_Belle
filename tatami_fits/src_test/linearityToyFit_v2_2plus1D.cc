...
//----------------------------------------------------------------------------------------------//
	...
	*/
	...
	//Combining P.D.F.'s
	RooAddPdf * fullSig1D = new RooAddPdf("fullSig1D","fullSig1D", RooArgList(*scfDT, *signalDT), RooArgList(f_scf));
	RooAddPdf * fullBkg1D = new RooAddPdf("fullBkg1D","fullBkg1D", RooArgList(*rareDT, *bbDT, *contDT), RooArgList(f_rare, f_bb));
	RooAddPdf * combined1D = new RooAddPdf("combined","combined", RooArgList(*fullSig1D, *fullBkg1D), RooArgList(Nsigscf, Nbkgtot));
...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fullFit = combined1D->fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	...
	combined1D->plotOn(xframe_mbc,Components("signalMBC"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data));
	combined1D->plotOn(xframe_mbc, LineColor(kBlue), ProjWData(*data));
	...
	combined1D->plotOn(xframe_de, Components("signalDE"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data));
	combined1D->plotOn(xframe_de, LineColor(kBlue), ProjWData(*data));
	...
	*/
	...
	combined1D->plotOn(xframe_dt,Components("signalDT"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data));
        combined1D->plotOn(xframe_dt, LineColor(kBlue), ProjWData(*data));
