...
int main(int argc, char *argv[]) {
	...
	tree->Add("/home/belle/varghese/IPHC/B_Kspipigamma_belle2/first_fits/rootfiles/RecoDecaysMC_CSMVA_cut.root");	
	...
	RooRealVar dtErr("dtErr", "dtErr", 0, 5);
	RooRealVar dummy("dummy", "dummy", 0, 5);
	...
	if(abs(d_dt) < 10){
	if(d_delE > -0.2 && d_delE < 0.2){

	dummy.setVal(0.0);
	dtErr.setVal(d_dterr/1000.0);
	dt.setVal(d_dt);
	inexp.setIndex(7);
...
//----------------------------------------------------------------------------------------------//
	...
	RooRealVar contDT_tau("contDT_tau", "contDT_tau", 1.4, 0.5, 1.8);
        ...
        RooRealVar contDT_S_main("contDT_S_main", "contDT_S_main", 1.7924, 0.4, 20);
