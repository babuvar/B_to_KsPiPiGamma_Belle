...
int main(int argc, char *argv[]) {
	...
	const double delta_m = 0.507; // from decay.xml in evtgen repo
	const double tau_b = 1.525; // from c*tau value in evt.pdl in evtgen repo and value of c in geatn4 repo
	...
	//Mistag rates for the current toy-file for signal events
	float dw_trueToy[7][2], w_trueToy[7][2], w_p_trueToy[7][2], w_m_trueToy[7][2];
	int ntrue_p[7][2] ={0}, nfalse_p[7][2] ={0}, ntrue_m[7][2]={0}, nfalse_m[7][2]={0};

	for(int i=0; i<tree->GetEntries(); i++){
	
	tree->GetEntry(i);
	if(f_issignal == 1){//issignal

	int expbin = int(i_inexp/30);
	//cout<<"f_genBFlavor = "<<f_genBFlavor<<endl;
	//cout<<"i_iflavor = "<<i_iflavor<<endl;
	if(f_genBFlavor == 1 && i_iflavor == 1) ntrue_p[i_irbin][expbin]++;
	if(f_genBFlavor == 1 && i_iflavor == -1) nfalse_p[i_irbin][expbin]++;
	if(f_genBFlavor == -1 && i_iflavor == -1) ntrue_m[i_irbin][expbin]++;
	if(f_genBFlavor == -1 && i_iflavor == 1) nfalse_m[i_irbin][expbin]++;
...
for(int i = 0; i < 7; i++){
for(int j = 0; j < 2; j++){
cout<<"nfalse_p = "<<nfalse_p[i][j]<<"\t";
cout<<"nfalse_m = "<<nfalse_m[i][j]<<"\t";
cout<<"ntrue_p = "<<ntrue_p[i][j]<<"\t";
cout<<"ntrue_m = "<<ntrue_m[i][j]<<"\t";


w_p_trueToy[i][j] = float(nfalse_p[i][j])/(float(ntrue_p[i][j])+float(nfalse_p[i][j]));
w_m_trueToy[i][j] = float(nfalse_m[i][j])/(float(ntrue_m[i][j])+float(nfalse_m[i][j]));
w_trueToy[i][j] = ( w_p_trueToy[i][j] + w_m_trueToy[i][j] )/2;
dw_trueToy[i][j] = w_p_trueToy[i][j] - w_m_trueToy[i][j];
}
cout<<endl;
}

	//Corrected mistag rates (from looking at MC before hand)
	...
	*/
	...
	//iflavor.setIndex(f_genBFlavor);
	iflavor.setIndex(i_iflavor);
	
	//correct avg-values from MC-truth
	int exp_bin = int(i_inexp/30);
	//w.setVal(w_trueToy[i_irbin][exp_bin]);
	//dw.setVal(dw_trueToy[i_irbin][exp_bin]);
	//cout<<"w = "<<w_trueToy[i_irbin][exp_bin]<<"\t dw-MC = "<<dw_trueToy[i_irbin][exp_bin]<<endl;	
        w.setVal(w_true[i_irbin][exp_bin]);
        dw.setVal(dw_true[i_irbin][exp_bin]);
	//cout<<"w-belle = "<<f_w<<"\t w-MC = "<<w_true[i_irbin][exp_bin]<<endl;
	//cout<<"dw-belle = "<<f_dw<<"\t dw-MC = "<<dw_true[i_irbin][exp_bin]<<endl;
	//w.setVal(f_w);
	//dw.setVal(f_dw);
	//w.setVal(0.0);
	//dw.setVal(0.0);
...
//----------------------------------------------------------------------------------------------//
	...
	confilename = "/A"; confilename = ConResDir + confilename + fileSuffix;
        ...
        fout<<toyindex<<"\t"<<A.getVal()<<"\t"<<A.getError()<<"\t"<<0.0<<"\t"<<fullFit->status()<<endl ;
        fout.close();
        //S
        confilename = "/S"; confilename = ConResDir + confilename + fileSuffix;
        fout.open(confilename, ios::app);
        fout<<toyindex<<"\t"<<S.getVal()<<"\t"<<S.getError()<<"\t"<<0.0<<"\t"<<fullFit->status()<<endl ;
        fout.close();
