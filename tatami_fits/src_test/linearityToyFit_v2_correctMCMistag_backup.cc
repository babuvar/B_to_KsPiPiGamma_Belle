...
*/	
	...
	RooRealVar Nbkgtot("N_{allbkg}","Combined background yield",4650, 1500, 6000);
...
//----------------------------------------------------------------------------------------------//
	...
	//Dummy p.d.f. for plotting purposes only
	RooFormulaVar Nscf("Nscf", "@0 * @1", RooArgList(Nsigscf, f_scf));
        RooAddPdf * allBKG = new RooAddPdf("allBKG", "allBKG", RooArgList(*scf3D, *fullBkg3D), RooArgList(Nscf, Nbkgtot));
	int Nallbkg = (Nscf.getVal() + Nbkgtot.getVal())/2;
	...
	//Signal only dt-projections
	//Get data-allbkg residuals
	auto entries_p = data->sumEntries("iflavor==iflavor::positive");
	auto entries_n = data->sumEntries("iflavor==iflavor::negative");	

	RooPlot *frame_dt = dt.frame(Bins(40));	
	data->plotOn(frame_dt, Cut("iflavor==iflavor::positive"));
	//combined3D->plotOn(frame_dt, Components("fullBkg3D"), ProjWData(*data), LineColor(kBlue), Normalization(entries_p, RooAbsReal::NumEvent));
	allBKG->plotOn(frame_dt, ProjWData(*data), LineColor(kBlue), Normalization(Nallbkg, RooAbsReal::NumEvent));
	RooHist* sigDt_p = frame_dt->residHist();
	data->plotOn(frame_dt, Cut("iflavor==iflavor::negative"));
	allBKG->plotOn(frame_dt, ProjWData(*data), LineColor(kBlue), Normalization(Nallbkg, RooAbsReal::NumEvent));
	RooHist* sigDt_n = frame_dt->residHist();
	//allBKG
	RooPlot *frame_dt_sig = dt.frame(Bins(40));	
	frame_dt_sig->addPlotable(sigDt_p, MarkerColor(kGreen));
	frame_dt_sig->addPlotable(sigDt_n, MarkerColor(kViolet));
	//combined3D->plotOn(frame_dt_sig, Components("signalDT"), Slice(iflavor, "positive"), MarkerColor(kGreen));	
	//combined3D->plotOn(frame_dt_sig, Components("signalDT"), Slice(iflavor, "negative"), MarkerColor(kViolet));


	//dt - asymmetry
	TF1 *f1 = new TF1("myfunc","(([0]*sin(0.507*x)) + ([1]*cos(0.507*x)))",-10, 10);
	RooArgSet pars(*rws.var("S"), *rws.var("A"));	
	//RooAbsReal *osc = bindFunction("osc", f1, dt, pars);
	//RooAbsReal *osc = bindFunction("osc", f1, dt, *rws.var("S"), *rws.var("A"));
	RooPlot *frame_asy = dt.frame(Bins(40),Title("Asymmetry"));
	data->plotOn(frame_asy, Asymmetry(iflavor));
	//osc->plotOn(frame_asy);
	//combined3D->plotOn(xframe_dt,Components("signalDT"), Asymmetry(iflavor));
	//combined3D->plotOn(xframe_dt,Asymmetry(iflavor), ProjWData(*data->reduce("irbin>irbin::0")), LineColor(kBlue));
	//combined3D->plotOn(frame_asy, Asymmetry(iflavor), LineColor(kBlue));
	...
	pad22->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_de->Draw() ;
        pad31->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); frame_dt_sig->Draw() ;// xframe_dt->Draw() ;
