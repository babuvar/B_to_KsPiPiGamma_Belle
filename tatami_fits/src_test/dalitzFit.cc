...
int main(int argc, char *argv[]) {
	...
	AllConditionalVariables.add(dalitzCategory);

        RooArgSet mainVariables(mbc, de, dt);
...
//----------------------------------------------------------------------------------------------//
        ...
        tree->GetEntry(i);
	//Splitting as {sig+scf} & {cont+rarebkg+bb}
	if(f_issgnevt== 1){ true_Nsigscf++; }
        else{ true_Nbkgtot++; }

	//Individual components
	if(f_issgnevt == 1 && f_issignal == 1){true_Nsig++;}
	if(f_issgnevt == 1 && f_issignal != 1){true_Nscf++;}
	if(f_mc_type == 20 || f_mc_type == 40){true_Ncont++;}
	if(f_mc_type == 10 || f_mc_type == 30){true_Nbb++;}	
	if(f_mc_type == 0 && f_issgnevt != 1){ true_Nrare++; }
        }
...
*/	
	...
	RooRealVar Nsigscf("Nsigscf","Signal + SCF yield", 840, 300, 1500);
	RooRealVar Nbkgtot("Nbkgtot","Combined background yield",4650, 1500, 6000);
	...
	//RooAddPdf Combined3D("Combined3D","Combined3D", RooArgList(*fullSig3D, *fullBkg3D), RooArgList(Nsigscf, Nbkgtot));

	
	//Construct simultaneous fit for 2 dalitz regions
	//WAY-1
	//RooWorkspace rws2("rws2", "rws2");
	//rws2.import(RooArgSet(Combined3D, dalitzCategory));
	//RooSimWSTool sct(rws2);
	//RooSimultaneous *COmbined3D = sct.build("COmbined3D", "Combined3D", SplitParam("Nsigscf, Nbkgtot", "dalitzCategory"));
	//RooAbsPdf *combined3D = (RooAbsPdf*)COmbined3D;
	//WAY-1

	//WAY-2
	RooSimPdfBuilder builder(*Combined3D) ;
	RooArgSet* config = builder.createProtoBuildConfig();
	config->setStringValue("physModels","Combined3D") ;
	config->setStringValue("splitCats" ,"dalitzCategory") ;
	config->setStringValue("Combined3D", "dalitzCategory : S, Nsigscf, Nbkgtot");
	RooSimultaneous* COmbined3D = builder.buildPdf(*config,data);
	RooAbsPdf * combined3D = (RooAbsPdf*)COmbined3D;
...
//----------------------------------------------------------------------------------------------//
	...
	auto entries_p = data->sumEntries("dalitzCategory==dalitzCategory::Plus");
	auto entries_n = data->sumEntries("dalitzCategory==dalitzCategory::Minus");
	...
	//combined3D->plotOn(xframe_mbc,Components("signalMBC"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_p, RooAbsReal::NumEvent));
	combined3D->plotOn(xframe_mbc, LineColor(kBlue), ProjWData(*data), Normalization(entries_p, RooAbsReal::NumEvent));
	...
	//combined3D->plotOn(xframe_de, Components("signalDE"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_p, RooAbsReal::NumEvent));
	combined3D->plotOn(xframe_de, LineColor(kBlue), ProjWData(*data), Normalization(entries_p, RooAbsReal::NumEvent));
	...
	//combined3D->plotOn(xframe_dt, Components("signalDT"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_p, RooAbsReal::NumEvent));
        combined3D->plotOn(xframe_dt, Slice(dalitzCategory,"Plus"), LineColor(kBlue), ProjWData(dalitzCategory, *data), Normalization(entries_p, RooAbsReal::NumEvent));
	...
	// DALITZ MINUS	
	//Mbc
	RooPlot *xframe_mbc2 = mbc.frame(Bins(40));
        data->plotOn(xframe_mbc2, Cut("dalitzCategory==dalitzCategory::Minus"));
        //combined3D->plotOn(xframe_mbc2, Components("signalMBC"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        combined3D->plotOn(xframe_mbc2, LineColor(kBlue), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        RooHist* pull_mbc2 = xframe_mbc2->pullHist() ;
        RooPlot* f_pull_mbc2 = mbc.frame(Title("")) ;
        f_pull_mbc2->addPlotable(pull_mbc2,"BX0") ;
        f_pull_mbc2->SetMaximum(4);
        f_pull_mbc2->SetMinimum(-4);
        f_pull_mbc2->GetYaxis()->SetLabelSize(0.11);
        f_pull_mbc2->GetXaxis()->SetLabelSize(0.11);


        //dE
        RooPlot *xframe_de2 = de.frame(Bins(40));
        data->plotOn(xframe_de2, Cut("dalitzCategory==dalitzCategory::Minus"));
        //combined3D->plotOn(xframe_de2, Components("signalDE"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        combined3D->plotOn(xframe_de2, LineColor(kBlue), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        RooHist* pull_de2 = xframe_de2->pullHist() ;
        RooPlot* f_pull_de2 = de.frame(Title("")) ;
        f_pull_de2->addPlotable(pull_de2,"BX0") ;
        f_pull_de2->SetMaximum(4);
        f_pull_de2->SetMinimum(-4);
        f_pull_de2->GetYaxis()->SetLabelSize(0.11);
        f_pull_de2->GetXaxis()->SetLabelSize(0.11);      

        //dt	
	RooPlot *xframe_dt2 = dt.frame(Bins(40));
        data->plotOn(xframe_dt2, Cut("dalitzCategory==dalitzCategory::Minus"));
        //combined3D->plotOn(xframe_dt2,Components("signalDT"), LineColor(kRed), LineStyle(kDashed), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        combined3D->plotOn(xframe_dt2, LineColor(kBlue), ProjWData(*data), Normalization(entries_n, RooAbsReal::NumEvent));
        RooHist* pull_dt2 = xframe_dt2->pullHist() ;
        RooPlot* f_pull_dt2 = dt.frame(Title("")) ;
        f_pull_dt2->addPlotable(pull_dt2,"BX0") ;        
        f_pull_dt2->SetMaximum(6);
        f_pull_dt2->SetMinimum(-6);
        f_pull_dt2->GetYaxis()->SetLabelSize(0.11);
        f_pull_dt2->GetXaxis()->SetLabelSize(0.11);
	...
	pad1->cd();
	TPad *pad11 = new TPad("pad11", "pad 11",0.0,0.6,1.0,1.0,0);
	TPad *pad12 = new TPad("pad12", "pad 12",0.0,0.5,1.0,0.6,0);
	TPad *pad13 = new TPad("pad13", "pad 13",0.0,0.1,1.0,0.5,0);
	TPad *pad14 = new TPad("pad14", "pad 14",0.0,0.0,1.0,0.1,0);
	pad11->Draw(); pad12->Draw(); pad13->Draw(); pad14->Draw();	
	pad2->cd();
	TPad *pad21 = new TPad("pad21", "pad 21",0.0,0.6,1.0,1.0,0);
        TPad *pad22 = new TPad("pad22", "pad 22",0.0,0.5,1.0,0.6,0);
        TPad *pad23 = new TPad("pad23", "pad 23",0.0,0.1,1.0,0.5,0);
        TPad *pad24 = new TPad("pad24", "pad 24",0.0,0.0,1.0,0.1,0);
	pad21->Draw(); pad22->Draw(); pad23->Draw(); pad24->Draw();
	pad3->cd();
        TPad *pad31 = new TPad("pad31", "pad 31",0.0,0.6,1.0,1.0,0);
        TPad *pad32 = new TPad("pad32", "pad 32",0.0,0.5,1.0,0.6,0);
        TPad *pad33 = new TPad("pad33", "pad 33",0.0,0.1,1.0,0.5,0);
        TPad *pad34 = new TPad("pad34", "pad 34",0.0,0.0,1.0,0.1,0);
	pad31->Draw(); pad32->Draw(); pad33->Draw(); pad34->Draw();
	...
	pad13->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_mbc2->Draw() ;
	pad14->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_mbc2->Draw() ;
	...
	pad23->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_de2->Draw() ;
	pad24->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_de2->Draw() ;
	...
	pad33->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); xframe_dt2->Draw() ;
	pad34->cd(); gPad->SetLeftMargin(0.15) ; gPad->SetBottomMargin(0.15); f_pull_dt2->Draw() ;
	...
	RooArgList rr = fullFit->floatParsFinal();
	cout<<"parameter size = "<<rr.getSize()<<endl;
	for(int i=0;i<rr.getSize(); i++)
	{
		RooRealVar* rrv = dynamic_cast<RooRealVar*>(&rr[i]) ;
		cout<<rrv->GetName()<<", "<<rrv->getVal()<<", "<<rrv->getError()<<endl;
	}
