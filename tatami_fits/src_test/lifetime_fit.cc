...
int main(int argc, char *argv[]) {
	...
	RooCategory dalitzCategory("dalitzCategory","Split by charge") ;
        ...
        RooRealVar tauB("\\tau_{B}", "tauB", 1.1, 2.0);
...
//----------------------------------------------------------------------------------------------//
	...
	RooDtLifetime * signalDT = new RooDtLifetime("DtSignalLifetime","DtLifetime",
                                dt, tauB, inexp, CosThetaB, ecms, ntrk_sig, z_err_sig, chisq_tracks_sig,
                                dgf_tracks_sig, ntrk_tag, z_err_tag, chisq_tracks_tag, dgf_tracks_tag,
                                keeptagl_tag, iflavor, w, dw, delta_m, true, true, 1.0);
...
//----------------------------------------------------------------------------------------------//
	...
	//tauB
	confilename = "/tauB"; confilename = ConResDir + confilename + fileSuffix;
	fout.open(confilename, ios::app);
	fout<<toyindex<<"\t"<<tauB.getVal()<<"\t"<<tauB.getError()<<"\t"<<0.0<<endl ;
	fout.close();
