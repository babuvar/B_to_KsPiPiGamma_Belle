...
int main(int argc, char *argv[]) {
	...
	tree->Add("/gpfs/home/belle/varghese/IPHC/B_Kspipigamma_belle/tatami_fits/alternate_way/rootfiles_for_shape/S0p0_1M_correct_toFit.root");
	...
	// Import ttree
	RooDataSet * data = new RooDataSet("data", "data", mainVariables, Import(*tree), Cut("(abs(dt) < 10.0) && (issignal != 1) && (issgnevt == 1) && (mcflags != 10)"));
...
//----------------------------------------------------------------------------------------------//
	...
	//MBC
	//Gaussian
	RooRealVar scfMBC_mean("scfMBC_mean", "scfMBC_mean", 5.28, 5.265, 5.290) ;
	RooRealVar scfMBC_sigmaL("scfMBC_sigmaL", "scfMBC_sigmaL", 0.0034532, 0.001, 0.1) ;
	RooRealVar scfMBC_sigmaR("scfMBC_sigmaR", "scfMBC_sigmaR", 0.0034532, 0.001, 0.02) ;
	RooBifurGauss scfMBC_Gauss("scfMBC_Gauss", "scfMBC_Gauss", mbc, scfMBC_mean, scfMBC_sigmaL, scfMBC_sigmaR) ;
        //Argus
        RooRealVar scfMBC_c("scfMBC_c", "scfMBC_c", -150., -190, -1) ;
        ...
        //combined
        RooRealVar scfMBC_f("scfMBC_f", "scfMBC_f", 0.25, 0., 0.99) ;
        RooAddPdf scfMBC("scfMBC", "scfMBC", RooArgList(scfMBC_Gauss, scfMBC_Argus), RooArgList(scfMBC_f)) ;	

	//dE
	//Chebychev 
	RooRealVar scfDE_p0("scfDE_p0", "scfDE_p0", 0.0, -10, 10);
	RooRealVar scfDE_p1("scfDE_p1", "scfDE_p1", 0.0, -10, 10);
	...
	RooRealVar scfDT_tau("scfDT_tau", "scfDT_tau", 1.40, 0.5, 1.8);
        ...
        //RooDtBkg scfDT("scfDT", "scfDT", dt, scfDT_tau, scfDT_mu_l, scfDT_mu_d, scfDT_f_delt, scfDT_f_tail, scfDT_S_main, scfDT_S_tail,                 inexp, ntrk_sig, z_err_sig, ntrk_tag, z_err_tag, true);

	const double tau_b = 1.0906;
	RooDtCPSignal scfDT("scfDT", "scfDT", dt, S, A, inexp, CosThetaB, ecms, ntrk_sig, z_err_sig, chisq_tracks_sig, dgf_tracks_sig, ntrk_tag, z_err_tag, chisq_tracks_tag, dgf_tracks_tag, keeptagl_tag, iflavor, w, dw, delta_m, tau_b, true, true, 1.0);


	//RooDtLifetime scfDT("scfDT","scfDT", dt, tauB, inexp, CosThetaB, ecms, ntrk_sig, z_err_sig, chisq_tracks_sig, dgf_tracks_sig, ntrk_tag, z_err_tag, chisq_tracks_tag, dgf_tracks_tag, keeptagl_tag, iflavor, w, dw, delta_m, true, true, 1.0);
	...
	scfMBC_mean.setConstant(true);
        scfMBC_sigmaL.setConstant(true);
	scfMBC_sigmaR.setConstant(true);
	scfMBC_f.setConstant(true);
	scfMBC_c.setConstant(true);
	scfMBC_m0.setConstant(true);
	//dE
	scfDE_p0.setConstant(true);
	scfDE_p1.setConstant(true);
	//dt
...
//----------------------------------------------------------------------------------------------//
	...
	can->SaveAs("plots/fix_shape_new/scf_shape.png");
	...
	*/
	//Save fit results
	RooArgSet(fitMbcbkg->floatParsFinal()).writeToFile("results/fix_shape_new/scf_Mbc.txt");
	RooArgSet(fitDebkg->floatParsFinal()).writeToFile("results/fix_shape_new/scf_dE.txt");
	RooArgSet(fitDtbkg->floatParsFinal()).writeToFile("results/fix_shape_new/scf_dt.txt");
	...
	rws->import(scfMBC); 
        rws->import(scfDE);
        rws->import(scfDT);
	rws->writeToFile("pdf_models/fix_shape_new/scf_shape.root");
	...
	//Generate randomized parameters from RooFitResult and save in a root-file 
        ...
        w_randPars->writeToFile("RandomizedParameters/fix_shape_new/scf_randPars.root");
