...
//----------------------------------------------------------------------------------------------//
	...
	//Rare-bkg Mbc Model
	//CrystalBall
	RooRealVar mbcraremean("\\mu_{Rare}^{M_{bc}}","B^{#pm} mass", 5.2812, 5.275,5.285) ;
	RooRealVar mbcraresigma("\\sigma_{Rare}^{M_{bc}}","B^{#pm} width",0.00300, 5e-4,5e-3);
	RooRealVar mbcrarealpha("\\alpha_{Rare}^{M_{bc}}","B^{#pm} width", 0.25, 0.,5.);
	RooRealVar mbcraren("n_{Rare}^{M_{bc}}","B^{#pm} width", 5, 0,20);
        RooCBShape mbcrareCBall("mbcrareCBall", "Crystal Ball shape", mbc, mbcraremean, mbcraresigma, mbcrarealpha, mbcraren);
	//Argus
	RooRealVar argparmbcRarec("argusRare_c","argus slope parameter", -44, -140,-1) ;
	RooRealVar argparmbcRarem0("argusRare_m0}","argus m0 parameter",5.29,5.28,5.30);
        RooArgusBG argusmbcRare("argusmbcRare","Argus PDF",mbc,argparmbcRarem0,argparmbcRarec) ;
	//Combined
	RooRealVar fmbcRarebkg("f_{Rare}^{M_{bc}}","#signal events", 0.29, 0., 1.0) ;
	RooAddPdf sumMbcRare("rareMBC","g+a2",RooArgList(mbcrareCBall,argusmbcRare),RooArgList(fmbcRarebkg)) ;


	//Rare-bkg dE Model
	RooRealVar dealphaRarebkg("\\alpha_{Rare}^{\\Delta E}","dealphaRarebkg", -3.62, -10.,0.1); 
	RooExponential expoRarebkg("rareDE","background p.d.f.",de,dealphaRarebkg);

	//Rare-bkg dt Model
	RooRealVar tau_Rarebkg("\\tau_{Rare}", "tau_bkgRare", 1.4, 0.5,1.8); 
        RooRealVar mu_lgRare("\\mu_{l}^{Rare}", "mu_lRare", -0.7,-0.8,0.5); 
        RooRealVar mu_dgRare("\\mu_{d}^{Rare}", "mu_dRare", -0.079, -0.1,0.1); 
        RooRealVar f_deltgRare("f_{delt}^{Rare}", "f_deltRare", 0.15, 0.1,1); 
        RooRealVar f_tailgRare("f_{tail}^{Rare}", "f_tailRare", 0.322, 0, 1.);
        RooRealVar S_maingRare("S_{main}^{Rare}", "S_mainRare", 2.487, 0.4,10);
        RooRealVar S_tailgRare("S_{tail}^{Rare}", "S_tailRare", 3.31, 1, 40); 


        RooDtBkg dtRarebkg("rareDT", "DtBkg", dt,
		tau_Rarebkg, mu_lgRare, mu_dgRare, f_deltgRare, f_tailgRare, S_maingRare, S_tailgRare,
                inexp, ntrk_sig, z_err_sig, ntrk_tag, z_err_tag, true);

	//3D PDF
	RooProdPdf * rare3D = new RooProdPdf("rare3D","rare3D",RooArgList(sumMbcRare, expoRarebkg, dtRarebkg));

	//If parameters are fixed
	if(fixedparameters == true){
        //Mbc
	mbcraremean.setConstant(true);
        mbcraresigma.setConstant(true);
        mbcrarealpha.setConstant(true);
        mbcraren.setConstant(true);
	argparmbcRarec.setConstant(true);
	argparmbcRarem0.setConstant(true);
	fmbcRarebkg.setConstant(true);
        //dE
	dealphaRarebkg.setConstant(true);
        //dt
	tau_Rarebkg.setConstant(true);
	mu_lgRare.setConstant(true);
	mu_dgRare.setConstant(true);
	f_deltgRare.setConstant(true);
	f_tailgRare.setConstant(true);
	S_maingRare.setConstant(true);
	S_tailgRare.setConstant(true);
	}
...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fit3D = rare3D->fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	...
	sumMbcRare.plotOn(xframe_mbc, LineColor(kBlue), ProjWData(*data));
	//sumMbcRare.paramOn(xframe_mbc, ProjWData(*data));	
	...
	expoRarebkg.plotOn(xframe_de, LineColor(kBlue), ProjWData(*data));
	//expoRarebkg.paramOn(xframe_de, ProjWData(*data));
	...
	RooPlot *xframe_dt = dt.frame(Bins(40));
        ...
        dtRarebkg.plotOn(xframe_dt, LineColor(kBlue), ProjWData(*data));
        //dtRarebkg.paramOn(xframe_dt, ProjWData(*data));
	...
	can->SaveAs("plots/fix_shape/rarebkg_shape3D.png");

	//Save fit results
	RooArgSet(fit3D->floatParsFinal()).writeToFile("results/fix_shape/Rare_3D.txt");

	//Save fit_model in RooWorkspace root file
	RooWorkspace *rws = new RooWorkspace("rare", "rare");
	rws->import(sumMbcRare);
        rws->import(expoRarebkg);
        rws->import(dtRarebkg);
	rws->writeToFile("pdf_models/fix_shape/rarebkg_shape3D.root");
