...
//----------------------------------------------------------------------------------------------//
	...
	//3d PDF
	RooProdPdf * scf3D = new RooProdPdf("scf3D","scf3D",RooArgList(sumMbcCF, sumCFbkg, DtBkgCF));
...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fit3D = scf3D->fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	...
	//Mbc-DE 2D
	TH2F * mbc_de_datahist = (TH2F *)data->createHistogram("mbc,de", 30, 30);
        TH2F * mbc_de_pdfhist = (TH2F *)scf3D->createHistogram("mbc,de", 30, 30);
        mbc_de_pdfhist->Scale(mbc_de_datahist->GetEntries());
        TH2F * mbc_de_resid = new TH2F("residual","residual",30,5.2, 5.3, 30, -0.2, 0.2);
        mbc_de_resid->Add(mbc_de_datahist, 1.0);
        mbc_de_resid->Add(mbc_de_pdfhist, -1.0);
        TH2F * mbc_de_pull = new TH2F("pull","pull",30,5.2, 5.3, 30, -0.2, 0.2);
        for(int i = 0; i< 30; i++){
        for(int j= 0; j< 30; j++){
	...
	can->SaveAs("plots/fix_shape/scf_shape3D.png");
	...
	gStyle->SetOptStat(0);
        ...
        can2->SaveAs("plots/fix_shape/scf_shape3D_MbcDe.png");

	//Save fit results
	RooArgSet(fit3D->floatParsFinal()).writeToFile("results/fix_shape/scf_3D.txt");
	...
	rws->import(sumCFbkg);
	rws->import(DtBkgCF);
	rws->writeToFile("pdf_models/fix_shape/scf_shape3D.root");
