...
//----------------------------------------------------------------------------------------------//
	...
	//MBC
	//Argus
	RooRealVar bbMBC_c("bbMBC_c","bbMBC_c", -8.11070,-140,2) ;
        RooRealVar bbMBC_m0("bbMBC_m0","bbMBC_m0", 5.2885, 5.28, 5.30) ;
	RooArgusBG bbMBC_argus("bbMBC_argus","bbMBC_argus", mbc, bbMBC_m0, bbMBC_c);        //Gaussian
	RooRealVar bbMBC_mu("bbMBC_mu", "bbMBC_mu", 5.2749, 5.270,5.290) ;
        RooRealVar bbMBC_sig("bbMBC_sig", "bbMBC_sig", 0.0020, 0.001, 0.05) ;
	...
	RooRealVar bbMBC_f("bbMBC_f","bbMBC_f", 0.0); //, 0.0 ,0.9) ;
        ...
        //dE
        RooRealVar bbDE_al("bbDE_al", "bbDE_al", -6.01751, -10., 0.) ;
        RooExponential bbDE("bbDE", "bbDE", de, bbDE_al) ;

        //dt
        RooRealVar bbDT_tau("bbDT_tau", "bbDT_tau", 1.40, 0.5, 1.8);
        RooRealVar bbDT_mu_l("bbDT_mu_l", "bbDT_mu_l", -0.698928, -0.7, 0.5);
        RooRealVar bbDT_mu_d("bbDT_mu_d", "bbDT_mu_d", 0.0028751, -0.1, 0.1);
        RooRealVar bbDT_f_delt("bbDT_f_delt", "bbDT_f_delt", 0.150, 0.1,1);
        RooRealVar bbDT_f_tail("bbDT_f_tail", "bbDT_f_tail", 0.19111, 0, 1);
        RooRealVar bbDT_S_main("bbDT_S_main", "bbDT_S_main", 2.8550, 0.4, 10);
        RooRealVar bbDT_S_tail("bbDT_S_tail", "bbDT_S_tail", 3.4377, 1, 40);
	...
	//Mbc
	bbMBC_c.setConstant(true);
	bbMBC_m0.setConstant(true);
	bbMBC_mu.setConstant(true);
	bbMBC_sig.setConstant(true);
	bbMBC_f.setConstant(true);
	//dE
	bbDE_al.setConstant(true);
	//dt
	bbDT_tau.setConstant(true);
	bbDT_mu_l.setConstant(true);
	bbDT_mu_d.setConstant(true);
	bbDT_f_delt.setConstant(true);
	bbDT_f_tail.setConstant(true);
	bbDT_S_main.setConstant(true);
	bbDT_S_tail.setConstant(true);
	}
...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fitMbcbkg = bbMBC.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDebkg = bbDE.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDtbkg = bbDT.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	...
	bbMBC.plotOn(xframe_mbc, LineColor(kBlue), ProjWData(*data));
	...
	bbDE.plotOn(xframe_de, LineColor(kBlue), ProjWData(*data));
	...
	RooPlot *xframe_dt = dt.frame(Bins(40));
        ...
        bbDT.plotOn(xframe_dt, LineColor(kBlue), ProjWData(*data));
	...
	can->SaveAs("plots/fix_shape/bb_shape.png");
	...
	*/	
	...
	//Save fit results
	RooArgSet(fitMbcbkg->floatParsFinal()).writeToFile("results/fix_shape/bb_Mbc.txt");
	RooArgSet(fitDebkg->floatParsFinal()).writeToFile("results/fix_shape/bb_dE.txt");
	RooArgSet(fitDtbkg->floatParsFinal()).writeToFile("results/fix_shape/bb_dt.txt");
	...
	rws->import(bbMBC); 
        rws->import(bbDE);
        rws->import(bbDT);
	rws->writeToFile("pdf_models/fix_shape/bb_shape.root");
	...
	w_randPars->writeToFile("RandomizedParameters/fix_shape/bb_randPars.root");
