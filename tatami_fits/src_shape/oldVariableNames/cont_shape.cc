...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fitMbcbkg = argus.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDebkg = expodeBkg.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDtbkg = dtbkg.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	...
	can->SaveAs("plots/fix_shape/cont_shape.png");
	...
	*/
	...
	//Save fit results
	RooArgSet(fitMbcbkg->floatParsFinal()).writeToFile("results/fix_shape/cont_Mbc.txt");
	RooArgSet(fitDebkg->floatParsFinal()).writeToFile("results/fix_shape/cont_dE.txt");
	RooArgSet(fitDtbkg->floatParsFinal()).writeToFile("results/fix_shape/cont_dt.txt");
	...
	rws->import(argus); 
        ...
        rws->import(dtbkg);
	rws->writeToFile("pdf_models/fix_shape/cont_shape.root");
	...
	//Generate randomized parameters from RooFitResult and save in a root-file 
        ...
        w_randPars->writeToFile("RandomizedParameters/fix_shape/cont_randPars.root");
