...
//----------------------------------------------------------------------------------------------//
	...
	//combined
	RooRealVar fmbcbbbkg("f_{BBbkg}^{M_{bc}}","#signal events", 0.0);//, 0.0 ,0.9) ;
...
//----------------------------------------------------------------------------------------------//
	...
	//Fitting
	RooFitResult * fitMbcbkg = sumMbcBBbkg.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDebkg = expoDeBBbkg.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
	RooFitResult * fitDtbkg = DtBkgBB.fitTo(*data,ConditionalObservables(AllConditionalVariables), Save(), NumCPU(10, 0)) ;
