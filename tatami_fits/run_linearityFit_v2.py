...
if __name__ == '__main__':
	...
	for fitindex in fitindices:
		...
		#save plot in root file
		os.system('%s/exe/%s'%(work_dir, executable) + ' -f %s/toy_dataset_%s_%s.root'%(input_dir, smc_sample, fitindex) + ' -p %s/toy_fit_%s_%s.root'%(plot_dir, smc_sample, fitindex) + ' -r %s/result_toy_%s_%s.txt'%(result_dir, smc_sample, fitindex) + ' -i %s -c %s'%(fitindex, consolidated_results_dir) + ' -t %s   > %s/log_toy_%s_%s.txt'%(smc_sample, log_dir, smc_sample, fitindex))

		#testing
		#print('%s/exe/%s'%(work_dir, executable) + ' -f %s/toy_dataset_%s_%s.root'%(input_dir, smc_sample, fitindex) + ' -p %s/toy_fit_%s_%s.root'%(plot_dir, smc_sample, fitindex) + ' -r %s/result_toy_%s_%s.txt'%(result_dir, smc_sample, fitindex) + ' -i %s -c %s'%(fitindex, consolidated_results_dir) + ' -t %s   > %s/log_toy_%s_%s.txt'%(smc_sample, log_dir, smc_sample, fitindex))
